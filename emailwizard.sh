...
# Have a Raspberry Pi server with a static IP and DNS records (usually
...
install_packages="postfix postfix-pcre dovecot-imapd dovecot-pop3d dovecot-sieve ufw opendkim opendkim-tools spamassassin spamc net-tools fail2ban bind9-host"
...
postconf -e 'mydestination = $myhostname, $mydomain, mail, box1, localhost.localdomain, localhost, localhost.$mydomain'
...
[ ! -f /etc/fail2ban/jail.d/emailwizard.local ] && echo "[postfix]
enabled = true
[postfix-sasl]
enabled = true
[sieve]
enabled = true
[dovecot]
enabled = true" > /etc/fail2ban/jail.d/emailwizard.local
